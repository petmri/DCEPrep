...
def analyze(file_dir):
    ...
    ax0.hist(T1_gm_data, n_bins, range=(10, 4000),histtype='bar', alpha = 0.9, color = 'gray', label = 'gm')
    # ax0.hist(T1_csf_data, n_bins, range=(10, 4000), histtype='bar', color = 'cyan', label = 'csf')
    ax0.axvline(median(T1_wm_data[np.where(np.logical_and(T1_wm_data>0, T1_wm_data<4000))]), color = 'pink', linestyle = 'dashed')
    ax0.axvline(median(T1_gm_data[np.where(np.logical_and(T1_gm_data>0, T1_gm_data<4000))]), color = 'gray', linestyle = 'dashed')
    # ax0.axvline(mean(T1_csf_data[T1_csf_data > 0]), color = 'cyan', linestyle = 'dashed')
    min_ylim, max_ylim = ax0.get_ylim()
    ax0.text(median(T1_wm_data[T1_wm_data > 0])*.27, max_ylim*0.9, 'Median: {:.1f}'.format(median(T1_wm_data[np.where(np.logical_and(T1_wm_data>0, T1_wm_data<4000))])), color='pink')
    ax0.text(median(T1_gm_data[T1_gm_data > 0])*1.02, max_ylim*0.9, 'Median: {:.1f}'.format(median(T1_gm_data[np.where(np.logical_and(T1_gm_data>0, T1_gm_data<4000))])), color='gray')
    ax0.text(median(T1_wm_data[T1_wm_data > 0])*.27, max_ylim*0.8, 'stdev: {:.1f}'.format(pstdev(T1_wm_data[np.where(np.logical_and(T1_wm_data>0, T1_wm_data<4000))])), color='pink')
    ax0.text(median(T1_gm_data[T1_gm_data > 0])*1.02, max_ylim*0.8, 'stdev: {:.1f}'.format(pstdev(T1_gm_data[np.where(np.logical_and(T1_gm_data>0, T1_gm_data<4000))])), color='gray')
    ax0.legend()
